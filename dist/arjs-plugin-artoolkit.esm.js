class m {
  ...
  _onWorkerMessage(g) {
    const C = g && g.data !== void 0 ? g.data : g, { type: e, payload: I } = C || {};
    if (e === "ready")
      console.log("[Plugin] Worker ready"), this.workerReady = !0, this.core?.eventBus?.emit("ar:workerReady", {});
    else if (e === "detectionResult") {
      if (console.log("[Plugin] Received detectionResult:", I), !I || !Array.isArray(I.detections)) return;
      for (const A of I.detections) {
        const l = A.id, s = Date.now(), d = new Float32Array(A.poseMatrix || []), t = A.confidence ?? 0, c = A.corners ?? [], b = this._markers.get(l);
        !b || !b.visible ? (this._markers.set(l, { lastSeen: s, visible: !0, lostCount: 0 }), this.core.eventBus.emit("ar:markerFound", { id: l, poseMatrix: d, confidence: t, corners: c, timestamp: s })) : (b.lastSeen = s, b.lostCount = 0, this._markers.set(l, b), this.core.eventBus.emit("ar:markerUpdated", { id: l, poseMatrix: d, confidence: t, corners: c, timestamp: s }));
      }
    } else if (e === "getMarker") {
      try {
    ...
    } else if (e === "loadMarkerResult") {
      console.log("[Plugin] Received loadMarkerResult:", I);
      const { requestId: A, ok: l, error: s, markerId: d, size: t } = I || {};
      if (A !== void 0) {
        const c = this._pendingMarkerLoads.get(A);
        c && (this._pendingMarkerLoads.delete(A), l ? c.resolve({ markerId: d, size: t }) : c.reject(new Error(s || "Failed to load marker")));
      }
    } else e === "error" && (console.error("Artoolkit worker error", I), this.core?.eventBus?.emit("ar:workerError", I));
  }
  ...
  _sweepMarkers() {
    ...
    for (const [e, I] of this._markers.entries())
      g - (I.lastSeen || 0) > C && (this._markers.delete(e), this.core.eventBus.emit("ar:markerLost", { id: e, timestamp: g }));
  }
  ...
  async loadMarker(g, C = 1) {
    ...
    return console.log(`[Plugin] Loading marker: ${g} with size ${C}`), new Promise((e, I) => {
      const A = this._nextLoadRequestId++;
      this._pendingMarkerLoads.set(A, { resolve: e, reject: I });
      try {
        this._worker.postMessage({
          type: "loadMarker",
        ...
        this._pendingMarkerLoads.delete(A), I(new Error(`Failed to send loadMarker message: ${l.message}`));
      }
      setTimeout(() => {
...
export {
  ...
  a as convertModelViewToThreeMatrix
};
